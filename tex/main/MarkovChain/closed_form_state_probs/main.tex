...
% TODO: Generalise notation in entire section 
...
\begin{multicols}{2}
    % TODO: Generalise notation - number of servers, system capacity and parking capacity
    \begin{itemize}
...
% TODO: Generalise notation - change ambulance patients and other patients into class 1 and 2 patients
...
\begin{itemize}
    \item \textbf{Number of servers (\(C\)):} Affects the weight of all edges \((v_i, v_j) \in E\) in the Markov chain that correspond to a service rate. These edges have a weight of: 
    \begin{equation*}
...
Thus, it can be assumed that a Markov chain model \(M\) can be translated as a weighted directed graph \(G_M = (V, E)\) where \(V=S\) from equation \ref{eq:state_space} and \((v_i, v_j)\in E\) if and only if \(q_{v_i, v_j}>0\). Furthermore, the weights are given by:
\[
...
Additionally, let us denote the set of all spanning trees of \(G\) as \(T(G)\) and the subset of \(T(G)\) that includes only the spanning trees that are rooted at vertex \(v\) as \(T_v(G)\). 
...
By increasing the parking capacity of the system it can be observed that \(|T_{(0,0)}(G)|\) increases as well since more combinations of paths can be generated using the new edges and vertices. 
...
Thus, having investigated the effect of adding rows (increasing \(M\)) it remains to investigate the effect of adding columns (increasing \(N\)) and finding an expression for \(k(C,T,N)\).
...
Note here that the final value \(U_{p_1,p_2,p_3}\) corresponds to coefficients that are unknown and are currently investigated. 
...
\begin{figure}[h]
    \centering
    \scalebox{0.8}{\input{MarkovChain/closed_form_state_probs/example_model_1131/main.tex}} \vspace{0.8cm} \\
...
\begin{multicols}{2}
    \begin{figure}[H]
        \centering
        \scalebox{0.6}{\input{MarkovChain/closed_form_state_probs/spanning_trees_1131/main_0.tex}}
    \end{figure}
...
\begin{multicols}{2}
    \begin{figure}[H]
        \centering
        \scalebox{0.6}{\input{MarkovChain/closed_form_state_probs/spanning_trees_1131/main_1.tex}}
    \end{figure}
...
\begin{multicols}{2}
    \begin{figure}[H]
        \centering
        \scalebox{0.6}{\input{MarkovChain/closed_form_state_probs/spanning_trees_1131/main_2.tex}}
    \end{figure}
...
\begin{multicols}{2}
    \begin{figure}[H]
        \centering
        \scalebox{0.6}{\input{MarkovChain/closed_form_state_probs/spanning_trees_1131/main_3.tex}}
    \end{figure}
...
\subsubsection{Possibly useful theorem: Matrix-tree theorem for directed graphs (Kirchhoff's theorem) \cite{matrix-tree-theorem}: }
