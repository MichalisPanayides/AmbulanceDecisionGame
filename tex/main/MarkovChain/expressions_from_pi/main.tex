...
\vec{\lambda})\), is given by the following expression, where \(\vec{r}\) 
contains all \(k\) values of erlang distributions and \(\vec{\lambda}\) contains
all 
...
\subsubsection{Proof of equation \ref{eq:hypoexponential-expression-to-proof}}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \end{equation*}
        \item Prove true for \(l = x + 1\): \(\Bigg( \frac{\partial^x}{\partial t ^ x} 
        \left( \frac{-1}{t (t + \lambda_2)} \right) = 
