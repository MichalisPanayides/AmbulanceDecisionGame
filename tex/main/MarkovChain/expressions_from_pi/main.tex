...
\begin{equation} \label{eq:linear-general-cdf-hypoexponential}
    \begin{split}
        F(x) = &1 - \sum_{k=1}^{n} \sum_{l=0}^{k-1} (-1)^{k-1} {n \choose k} 
            {k-1 \choose l} \sum_{j=1}^{n} \sum_{s=1}^{j-1} e^{-x \lambda_s} 
