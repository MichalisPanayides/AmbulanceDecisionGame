...
Unlike the waiting time, the blocking time is only calculated for ambulance patients.  % TODO Modify this to no longer refer to ambulance/patients etc...
...
In order to not consider patients that will be lost to the system, the set of accepting  % TODO patients -> generic vocabulary
...
ignoring any arrivals. Here, the same approach is used but ignoring only ambulance  % TODO ambulance -> generic term
arrivals. That is because for the waiting time formula, once a patient enters the  % TODO patient/hospital -> generic term
hospital (i.e. starts waiting) any patient arriving after them will not affect their
pathway. That is not the case for blocking time. When an ambulance patient is blocked,  % TODO patient -> generic term
...
\begin{itemize}
    ...
    \item be because of an arrival of another individual of the first class denoted by \(p_o(u,v)\).  % TODO Align "first class" with whatever generic terms are used.
\end{itemize}
...
\begin{multicols*}{2}
    \scalebox{0.68}{\input{MarkovChain/expressions_from_pi/example_model_2362/main.tex}}
    \columnbreak
    \vspace*{0cm} 
    \begin{align}
...
Additionally, the above equations can be transformed into a linear system of the form \(Mx=y\) with:
