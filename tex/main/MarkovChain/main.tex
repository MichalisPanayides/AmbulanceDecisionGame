...
For other patients, whenever the model is at state \( (u,v) \), any incoming patient will proceed to arrive at state \( (u, v+1) \). Patients will then proceed to visit all other states until they reach one which has less than $C$ servers occupied (i.e. until a server becomes available). For simplicity, and computational efficiency, all states \( (u,v) \) were changed to \( (0, v) \) since the sojourn time that is calculated at each state is independent of \(u\). Thus, the expected waiting time of an ``other'' patient when they arrive at state \( (u,v) \) can be given by:

\begin{equation}
    w^{(o)}(u,v) = 
    \begin{cases} 
        0, \hspace{4.85cm} & \textbf{if } (u,v) \notin S_w \\
        c(0,v + 1) + w(0, v-1), & \textbf{otherwise} \\
    \end{cases}
\end{equation}

The equivalent time for ``ambulance'' patients is denoted in a slightly different manner. States \(s_a\) and \(s_n\) represent the \textit{arriving state} and the \textit{next state} to be visited by the recursive formula. The arriving state is the state that the individual will actually arrive in when the model is in state \((u,v)\) and the next state is the one that the patient will move down to. The \textit{arriving state} is calculated based on the threshold of the model. If the model is at a state where \( v < T \) then the arriving patient will arrive at state \( (u, v+1) \) otherwise at state \( (u+1, v) \) (i.e. move to the state to the left or the state below). Additionally, the \textit{next state} is calculated based on what row the model is currently on. If the model is in the first row then the next state to be visited is the one before otherwise, if the the model is in any other row (meaning that the arriving ambulance will be instantly blocked when it arrives) the \textit{next state} to be visited jumps to state \((0,T)\). This occurs to avoid all states that will have a zero waiting time and moves straight to the first row since no wait occurs for any ambulance patient in any row apart from the first one.
