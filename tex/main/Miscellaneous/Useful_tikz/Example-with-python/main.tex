\begin{figure}[h]
\centering
\begin{tikzpicture}[-, node distance = 1cm, auto]
\node[state] (u0v0) {(0,0)};
\node[state, right=of u0v0] (u0v1) {(0,1)};
\draw[->](u0v0) edge[bend left] node {\( \Lambda \)} (u0v1);
\draw[->](u0v1) edge[bend left] node {\(1\mu \)} (u0v0);
\node[state, right=of u0v1] (u0v2) {(0,2)};
\draw[->](u0v1) edge[bend left] node {\( \Lambda \)} (u0v2);
\draw[->](u0v2) edge[bend left] node {\(2\mu \)} (u0v1);
\node[state, right=of u0v2] (u0v3) {(0,3)};
\draw[->](u0v2) edge[bend left] node {\( \Lambda \)} (u0v3);
\draw[->](u0v3) edge[bend left] node {\(3\mu \)} (u0v2);
\node[state, below=of u0v3] (u1v3) {(1,3)};
\draw[->](u0v3) edge[bend left] node {\( \lambda^A \)} (u1v3);
\draw[->](u1v3) edge[bend left] node {\(3\mu \)} (u0v3);
\node[state, below=of u1v3] (u2v3) {(2,3)};
\draw[->](u1v3) edge[bend left] node {\( \lambda^A \)} (u2v3);
\draw[->](u2v3) edge[bend left] node {\(3\mu \)} (u1v3);
\node[state, below=of u2v3] (u3v3) {(3,3)};
\draw[->](u2v3) edge[bend left] node {\( \lambda^A \)} (u3v3);
\draw[->](u3v3) edge[bend left] node {\(3\mu \)} (u2v3);
\node[state, right=of u0v3] (u0v4) {(0,4)};
\draw[->](u0v3) edge[bend left] node {\( \lambda^o \)} (u0v4);
\draw[->](u0v4) edge[bend left] node {\(4\mu \)} (u0v3);
\node[state, right=of u1v3] (u1v4) {(1,4)};
\draw[->](u1v3) edge[bend left] node {\( \lambda^o \)} (u1v4);
\draw[->](u1v4) edge[bend left] node {\(4\mu \)} (u1v3);
\draw[->](u0v4) edge node {\( \lambda^A \)} (u1v4);
\node[state, right=of u2v3] (u2v4) {(2,4)};
\draw[->](u2v3) edge[bend left] node {\( \lambda^o \)} (u2v4);
\draw[->](u2v4) edge[bend left] node {\(4\mu \)} (u2v3);
\draw[->](u1v4) edge node {\( \lambda^A \)} (u2v4);
\node[state, right=of u3v3] (u3v4) {(3,4)};
\draw[->](u3v3) edge[bend left] node {\( \lambda^o \)} (u3v4);
\draw[->](u3v4) edge[bend left] node {\(4\mu \)} (u3v3);
\draw[->](u2v4) edge node {\( \lambda^A \)} (u3v4);
\node[state, right=of u0v4] (u0v5) {(0,5)};
\draw[->](u0v4) edge[bend left] node {\( \lambda^o \)} (u0v5);
\draw[->](u0v5) edge[bend left] node {\(4\mu \)} (u0v4);
\node[state, right=of u1v4] (u1v5) {(1,5)};
\draw[->](u1v4) edge[bend left] node {\( \lambda^o \)} (u1v5);
\draw[->](u1v5) edge[bend left] node {\(4\mu \)} (u1v4);
\draw[->](u0v5) edge node {\( \lambda^A \)} (u1v5);
\node[state, right=of u2v4] (u2v5) {(2,5)};
\draw[->](u2v4) edge[bend left] node {\( \lambda^o \)} (u2v5);
\draw[->](u2v5) edge[bend left] node {\(4\mu \)} (u2v4);
\draw[->](u1v5) edge node {\( \lambda^A \)} (u2v5);
\node[state, right=of u3v4] (u3v5) {(3,5)};
\draw[->](u3v4) edge[bend left] node {\( \lambda^o \)} (u3v5);
\draw[->](u3v5) edge[bend left] node {\(4\mu \)} (u3v4);
\draw[->](u2v5) edge node {\( \lambda^A \)} (u3v5);
\node[state, right=of u0v5] (u0v6) {(0,6)};
\draw[->](u0v5) edge[bend left] node {\( \lambda^o \)} (u0v6);
\draw[->](u0v6) edge[bend left] node {\(4\mu \)} (u0v5);
\node[state, right=of u1v5] (u1v6) {(1,6)};
\draw[->](u1v5) edge[bend left] node {\( \lambda^o \)} (u1v6);
\draw[->](u1v6) edge[bend left] node {\(4\mu \)} (u1v5);
\draw[->](u0v6) edge node {\( \lambda^A \)} (u1v6);
\node[state, right=of u2v5] (u2v6) {(2,6)};
\draw[->](u2v5) edge[bend left] node {\( \lambda^o \)} (u2v6);
\draw[->](u2v6) edge[bend left] node {\(4\mu \)} (u2v5);
\draw[->](u1v6) edge node {\( \lambda^A \)} (u2v6);
\node[state, right=of u3v5] (u3v6) {(3,6)};
\draw[->](u3v5) edge[bend left] node {\( \lambda^o \)} (u3v6);
\draw[->](u3v6) edge[bend left] node {\(4\mu \)} (u3v5);
\draw[->](u2v6) edge node {\( \lambda^A \)} (u3v6);
\end{tikzpicture}
\caption{Markov chain model with 4 servers}
\label{Exmple_model-4363}
\end{figure}