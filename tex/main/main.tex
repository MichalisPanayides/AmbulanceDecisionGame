...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}[-, node distance = 1cm, auto, every node/.style={scale=0.59}]
        ...
        \node[state, node distance = 1.3cm, minimum size=1.5cm, right=of three] (four) {(0,T+1)};
        \node[draw=none, node distance = 1.3cm, minimum size=1.5cm, right=of four] (five) {\dots};
        ...
        \node[state, node distance = 1.3cm, minimum size=1.5cm, right=of five] (after_five) {(0,C)};
        \node[state, node distance = 1.3cm, minimum size=1.5cm, right=of five_one] (after_five_one) {(1,C)};
        \node[state, node distance = 1.3cm, minimum size=1.5cm, right=of five_two] (after_five_two_right) {(2,C)};

        \node[draw=none, minimum size=1.5cm, below=of three_two] (after_three_two) {\vdots};
        \node[draw=none, minimum size=1.5cm, below=of four_two] (after_four_two) {\vdots};
        \node[draw=none, minimum size=1.5cm, below=of five_two] (after_five_two_down) {};
        
        \node[draw=none, node distance = 1.3cm, minimum size=1.5cm, right=of after_five_two_down] (after_five_two_down_right) {\vdots};


        \node[draw=none, minimum size=1.5cm, right=of after_five] (last) {\dots};
        \node[draw=none, minimum size=1.5cm, right=of after_five_one] (last_one) {\dots};
        \node[draw=none, minimum size=1.5cm, right=of after_five_two_right] (last_two) {\dots};
...
% \begin{equation}
%     S = S(T,C) = \left\{(u, v) \in \mathbb{N}_0^2\;\|\text{ if } v\leq T \text{ then } u = 0 \right\}
% \end{equation}
...
Having calculated the transition matrix $Q$ for a given set of parameters the probability vector $\pi$ needs to be considered. The vector $\pi$ is commonly used to study such stochastic systems and it's main purpose is to keep track of the probability of being at any given state. Thus, by the definition of the steady state vector $\pi$ the relationship between it and $Q$  is given by:
